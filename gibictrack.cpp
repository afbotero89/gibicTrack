...
uchar sendValues[7]={1,200,100,2005,6,7};
double signalRx[128];
//Variables IGTLink
...
bool GibicTrack::ConectarSensor()
{
    ...
    serial->setPortName("/dev/ttyACM0");//Se requiere saber con anterioridad el nombre asignado al puerto
...
void GibicTrack::readData()
{
    ...
    //std::string datos = Qdata.toStdString();
        ...
        if (vectorDatos.split(",").count() == 7){
            ...
            if(vectorDatos.split(",")[6]=="\r\n"){
                ...
                if (filtroN == 1){
                    ...
                    MadgwickAHRSupdate(gx/filtro, gy/filtro, gz/filtro, ax/filtro, ay/filtro, az/filtro, 0.0, 0.0, 0.0);
